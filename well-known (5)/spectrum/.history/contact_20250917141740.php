...
<div id="recaptcha-error" style="color: red; font-size: 14px; margin-top: 5px; display: none; margin-">
...
function closePopup() {
  popupContent.classList.add("hide"); // play fadeOut
  setTimeout(() => {
    popupOverlay.style.display = "none";
    popupContent.classList.remove("hide"); // reset for next open
  }, 300);
}
form.addEventListener("submit", function(e) {
  e.preventDefault();

  const recaptchaError = document.getElementById("recaptcha-error"); // the div under reCAPTCHA

  // 1. Check if reCAPTCHA is completed
  const recaptchaResponse = grecaptcha.getResponse();
  if (!recaptchaResponse) {
    recaptchaError.style.display = "block"; // show message
    return; // stop submission
  } else {
    recaptchaError.style.display = "none"; // hide message if verified
  }

  // 2. Continue with AJAX submission
